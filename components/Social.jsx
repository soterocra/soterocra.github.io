...
const SOCIAL_DEFAULT = [
  {
    key: 'linkedin', name: 'LinkedIn', handle: '@soterocra',
    href: 'https://www.linkedin.com/in/soterocra',
    color: '#0A66C2', highlight: true,
  },
];

function SocialIcon({ k, color }) {
  const common = { width: 22, height: 22, fill: 'none', stroke: color, strokeWidth: 1.6, strokeLinecap: 'round', strokeLinejoin: 'round' };
  if (k === 'linkedin') return (
    <svg viewBox="0 0 24 24" {...common} style={{ fill: color, stroke: 'none' }}>
      <path d="M4.98 3.5C4.98 4.88 3.87 6 2.5 6S0 4.88 0 3.5 1.12 1 2.5 1s2.48 1.12 2.48 2.5zM.22 8h4.56v14H.22V8zm7.93 0h4.37v1.91h.06c.61-1.15 2.1-2.36 4.32-2.36 4.62 0 5.47 3.04 5.47 7v7.45h-4.56v-6.6c0-1.57-.03-3.6-2.19-3.6-2.2 0-2.53 1.71-2.53 3.49V22H8.15V8z"/>
    </svg>
  );
  if (k === 'github') return (
  ...
  if (k === 'youtube') return (
    <svg viewBox="0 0 24 24" width="22" height="22" fill={color}>
      <path d="M23.5 6.2c-.3-1-1.1-1.8-2.1-2.1C19.5 3.5 12 3.5 12 3.5s-7.5 0-9.4.6c-1 .3-1.8 1.1-2.1 2.1C0 8.1 0 12 0 12s0 3.9.5 5.8c.3 1 1.1 1.8 2.1 2.1 1.9.5 9.4.5 9.4.5s7.5 0 9.4-.6c1-.3 1.8-1.1 2.1-2.1.5-1.9.5-5.8.5-5.8s0-3.9-.5-5.7zM9.6 15.5V8.5l6.4 3.5-6.4 3.5z"/>
    </svg>
  );
  if (k === 'email') return (
...
function Social() {
  ...
  const SOCIAL = [
    ...
    {
      key: 'github', name: 'GitHub', handle: '@soterocra',
      detail: t('social.github.detail'),
      href: 'https://github.com/soterocra', color: 'var(--fg-1)',
    },
    {
      key: 'youtube', name: 'YouTube', handle: '@soterocra',
      detail: t('social.youtube.detail'),
      href: 'https://www.youtube.com/@soterocra', color: '#FF0033',
    },
    {
      key: 'email', name: 'E-mail', handle: 'me@soterocra.dev',
      detail: t('social.email.detail'),
      href: 'mailto:me@soterocra.dev', color: 'var(--accent)',
    },
  ];
        ...
        <a href={featured.href} target="_blank" rel="noopener noreferrer"
          ...
          <div style={{
            ...
            <div style={{
              ...
              <SocialIcon k={featured.key} color={featured.color} />
            </div>
            <span style={{
              fontFamily: 'var(--font-mono)', fontSize: 11, fontWeight: 700,
              letterSpacing: '.1em', color: featured.color,
              padding: '4px 8px', borderRadius: 4,
              background: `color-mix(in srgb, ${featured.color} 12%, transparent)`,
            }}>{t('social.principal')}</span>
          </div>
          <div>
            <h3 style={{
              fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: 24,
              margin: '0 0 4px', color: 'var(--fg-1)', letterSpacing: '-.02em',
            }}>{featured.name}</h3>
            <div style={{
          ...
          <p style={{
            ...
            color: 'var(--fg-2)', margin: 0, maxWidth: 460,
          }}>{featured.detail}</p>
          <span style={{
            display: 'inline-flex', alignItems: 'center', gap: 8,
            fontFamily: 'var(--font-mono)', fontSize: 13, fontWeight: 600,
            color: featured.color, marginTop: 4,
          }}>
            {t('social.linkedin.cta')} <span>↗</span>
          </span>
        </a>
        <div style={{
          display: 'grid', gridTemplateRows: 'repeat(3, 1fr)', gap: 12,
        }}>
          {others.map(s => (
            <a key={s.key} href={s.href}
              ...
              <span style={{
                ...
                <SocialIcon k={s.key} color={s.color} />
              </span>
